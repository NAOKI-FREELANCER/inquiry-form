<?php
require_once(__DIR__ . '/../config/config.php');
$inquiry = new \inquiry\Inquiry();
$inquiry->register_inquiry();
?>

<!DOCTYPE html>
 <html lang="ja">
 <head>
   <meta charset="utf-8">
   <title>確認ページ</title>
   <link rel="stylesheet" href="../css/styles.css">
 </head>
 <body>
    <div class='confirm_msg'>
        <h3>問い合わせ内容の確認</h3>
        <p>件名：<?= h($_SESSION['values']['title']) ?></p>
        <p>お名前：<?= h($_SESSION['values']['name']) ?></p>
        <p>メールアドレス：<?= h($_SESSION['values']['email']) ?></p>
        <p>電話番号：<?= h($_SESSION['values']['phone']) ?></p>
        <p>お問い合わせ内容</p>
        <p><?= h($_SESSION['values']['content']) ?></p>
    </div>
    <div class='confirm_btn'>
        <form method="post">
            <button class='button' type="submit" formaction='inquiry.php'>修正</button>
            <input class='button' name='button_submit' type="submit" value="送信"/>
        </form>
    </div>
 </body>
 </html>